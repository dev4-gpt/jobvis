...
Data Science, and GenAI as primary families. BI, Java-heavy, generic analyst,
...
Remotive, and cache adapters. Results are deduplicated by stable posting
identity, ranked in bounded batches, normalized for employment/work mode/clearance
signals, then divided into primary, adjacent, and blocked/review-required
sections. Source diagnostics remain independent so a result set that came only
from Adzuna is visible as such.

## Tailoring and application safety

Tailoring chooses a resume persona (AI/ML and Data Science, GenAI/RAG, Computer
Vision, or MLOps/ML Platform) and keeps the existing corpus references, link
preservation, PDF artifacts, fabrication validator, and cover-letter quality
gate. Browser automation remains visible and review-gated. Jobvis can open an
ATS form and fill explicitly approved safe fields, but it has no submit action.
