...
- Keep project and employer headings faithful to the corpus. Do not turn a source title into metadata such as `operating system (Cursor, Python)`, append editor/tool names to a title, or invent a generic `Research Assistantship` heading. Use the real project name and put tools in bullets or skills.
...
- The cover letter must be 250–350 words after PDF extraction; target 275–325 words to leave a rendering margin. Open with a candidate-specific reason for this role, include 2–3 concrete achievements, metrics, tools, projects, or outcomes from the corpus, and address at least 2 specific requirements from the job description.
...
- The deterministic eligibility and role-bucket fields in the target-job block are authoritative. If the role is adjacent or review-required, keep the pack explicitly review-only and do not describe it as a primary AI/ML match. Never override a hard blocker; blocked roles should not reach this prompt.
...
- Never paste company research into the opening. Do not mention a mission, product slogan, or company statistic unless you can rewrite it as one short, grammatical sentence. If research is malformed or ambiguous, omit it and open with the candidate's evidence instead. Never write “because it calls for about”, duplicate the company name, or copy a research snippet verbatim.
