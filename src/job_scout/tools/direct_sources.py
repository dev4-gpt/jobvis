...
_BOARD_CACHE: dict[tuple, tuple[float, object]] = {}
_BOARD_LOCKS: dict[tuple, Lock] = {}
_CACHE_LOCK = Lock()


def _board_payload(method: str, url: str, timeout: float, **kwargs):
    """Single-flight board fetch shared by role queries for fifteen minutes."""
    key = (method, url)
    with _CACHE_LOCK:
        lock = _BOARD_LOCKS.setdefault(key, Lock())
    with lock:
        cached = _BOARD_CACHE.get(key)
        if cached and time.monotonic() - cached[0] < 900:
            return cached[1]
        response = getattr(httpx, method)(url, timeout=timeout, **kwargs)
        response.raise_for_status()
        payload = response.json()
        _BOARD_CACHE[key] = (time.monotonic(), payload)
        return payload


def _location_matches(requested: str | None, actual: str) -> bool:
    # Country-wide requests are checked by the candidate eligibility layer.
    if not requested or requested.lower() in {"us", "usa", "united states", "anywhere in the united states"}:
        return True
        ...
        for token in self.board_tokens:
            ...
            for row in rows:
                ...
                job = _base(
                    ...
                    work_mode=_work_mode(location_text + " " + description),
                    ats="greenhouse",
                )
                if (
                    _matches(query, title, description)
                    and _location_matches(location, job.location)
                    and (not remote or job.remote)
                ):
                    found.append(job)
                    if len(found) >= limit:
                        return found
    ...
    def fetch(self, query: str, location: str | None, country: str | None, remote: bool, limit: int) -> list[JobPosting]:
        ...
        for company in self.companies:
            ...
            for row in rows if isinstance(rows, list) else []:
                ...
                mode = _work_mode(row.get("workplaceType"))
                job = _base(
                    source=self.name,
                    record_id=row.get("id"),
                    title=title,
    ...
    def fetch(self, query: str, location: str | None, country: str | None, remote: bool, limit: int) -> list[JobPosting]:
        ...
        for board in self.boards:
            ...
            for row in rows if isinstance(rows, list) else []:
                ...
                application = _text(row.get("applyUrl")) or listing
                mode = _work_mode(row.get("workplaceType"), row.get("isRemote") is True)
                job = _base(
                    source=self.name,
